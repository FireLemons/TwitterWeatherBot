...
module.exports = {
  ...
  getSunUpDown (coordinates, date) {
    ...
    const solarPosition = this.solarPosition
    const n = solarPosition.getN(solarPosition.getJulianDate(dateCorrection))
    ...
    const eclipticLongitude = solarPosition.getSolarEclipticLongitude(solarPosition.getSolarMeanLongitude(n), solarPosition.getMeanAnomaly(n))
    const solarDeclination = solarPosition.getSolarDeclination(solarPosition.getAxialTilt(n), eclipticLongitude)
    // These equations probably don't expire
    const meanSolarNoon = solarPosition.getMeanSolarNoon(coordinates.long, n)
    const solarNoon = solarPosition.getSolarNoon(eclipticLongitude, solarPosition.getMeanAnomaly(meanSolarNoon), meanSolarNoon)

    const hourRatio = solarPosition.getSolarHourAngle(coordinates.lat, solarDeclination) / (2 * Math.PI)
    const sunrise = solarPosition.getDateFromJulian(solarNoon - hourRatio)
    const sunset = solarPosition.getDateFromJulian(solarNoon + hourRatio)
